...
export default function CreateEventForm() {
  ...
  const handleCreateEvent = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // TODO: Map walletAddress to adminUserId when User auth is implemented
      // For now, we'll create events without adminUserId (backend allows null)
      ...
      try {
        ...
        // Backend endpoint: POST /voting-event
        // Payload: { eventName, adminUserId (optional) }
        // Backend will auto-generate: eventId, groupMerkleRootHash, groupSize, groupLeafCommitments
        console.log('Attempting to connect to backend at:', BACKEND_API_URL);
        ...
        eventId = createdEvent.eventId; // Backend returns eventId as number

        console.log('Event created successfully via backend:', {
          eventId,
          eventName,
          groupMerkleRootHash: createdEvent.groupMerkleRootHash,
          groupSize: createdEvent.groupSize,
        });
      } catch (backendError) {
        // Backend not available - fallback to local mock mode
        console.warn('Backend not available, using local mock mode:', backendError);
        useBackend = false;

        // Generate local eventId (timestamp-based to ensure uniqueness)
        eventId = Date.now();

        console.log('Event created in mock mode:', {
          eventId,
          eventName,
          mode: 'local-mock',
        });
      ...
      // TODO: Generate admin token (secure random string) - will be replaced by backend auth later
      const adminToken = Array.from(crypto.getRandomValues(new Uint8Array(32)))
        .map(b => b.toString(16).padStart(2, '0'))
        .join('');

      // Store minimal event data in sessionStorage (temporary until backend auth)
      sessionStorage.setItem('createdEvent', JSON.stringify({
        ...
        walletAddress,
        createdAt: Date.now(),
        mockMode: !useBackend, // Flag to indicate mock mode
      }));
        ...
        <div className="bg-white rounded-3xl shadow-2xl p-8 md:p-10 border border-gray-100">
          ...
          <div className="text-center mb-12">
            <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
              <h1 className="text-2xl font-bold text-gray-900 mb-3">Create New Voting Event</h1>
